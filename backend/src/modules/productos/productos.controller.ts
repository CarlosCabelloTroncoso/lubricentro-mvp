...
import type { ListarProductosQuery } from './productos.schemas';

export async function listar(req: Request, res: Response): Promise<void> {
  const filtros = datosValidados<ListarProductosQuery>(req, 'query');
...
export async function ajustarStock(req: Request, res: Response): Promise<void> {
  ok(res, await service.ajustarStock(Number(req.params.id), req.body.cantidad));
}

export async function alertas(_req: Request, res: Response): Promise<void> {
  ok(res, await service.alertasStock());
}
